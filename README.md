...
- **A passkey or USB security-key prompt means you are logged out.** This one cost me time, so it is worth being precise about. LinkedIn throws a "register a security key" prompt on its sign-in page, and it is modal enough to look like a mysterious blocker in the middle of a run. It is not a random interruption. It is what a logged-out session looks like.

  The fix is to log in yourself, in the pane, as a human, and then start the audit. Do not ask an agent to dismiss it, suppress it, or script around it. Suppressing a WebAuthn prompt means disabling a browser security mechanism to get past an authentication screen, and neither you nor a tool you found on GitHub should be doing that to your own account. There is no version of this tool that needs it: the whole thing assumes you are already signed in.
